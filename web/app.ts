...
(claim proof-zero-is-even (Even 0))
(define-tactically proof-zero-is-even
  ((exact zero-is-even)))

(claim proof-one-is-odd (Odd 1))
(define-tactically proof-one-is-odd
  ((exact (add1-even->odd 0 zero-is-even))))

(claim proof-three-is-odd (Odd 3))
(define-tactically proof-three-is-odd
   ((exact (add1-even->odd 2
     (add1-odd->even 1
...
function initializeProofTreeVisualizer() {
  ...
  // Initialize proof picker
  const proofPicker = document.getElementById('proof-picker') as HTMLSelectElement;
  if (proofPicker) {
    proofPicker.addEventListener('change', (e) => {
      const selectedIndex = (e.target as HTMLSelectElement).value;
      if (selectedIndex !== "" && window.lastProofTrees) {
        updateProofTree(window.lastProofTrees[parseInt(selectedIndex)].tree);
      }
    });
  }
}

// Add global storage for last proof trees
declare global {
  interface Window {
    lastProofTrees?: { name: string; tree: ProofTreeData }[];
  }
}

function applyDiagnostics(monaco: typeof Monaco, editor: Monaco.editor.IStandaloneCodeEditor, payload: any) {
  const { diagnostics, pretty, proofTrees } = payload;
  ...
  window.lastProofTrees = proofTrees;
  const proofPicker = document.getElementById('proof-picker') as HTMLSelectElement;

  if (proofTrees && proofTrees.length > 0) {
    if (proofPicker) {
      proofPicker.innerHTML = '';
      proofTrees.forEach((pt: { name: string }, index: number) => {
        const option = document.createElement('option');
        option.value = index.toString();
        option.textContent = pt.name;
        proofPicker.appendChild(option);
      });
      proofPicker.style.display = 'block';
      proofPicker.value = "0"; // Select first by default
    }
    updateProofTree(proofTrees[0].tree);
  } else {
    if (proofPicker) {
      proofPicker.style.display = 'none';
    }
    updateProofTree(undefined);
  }
